...
import SelectMonthAndLine from "@/app/(user)/andon/report/_components/filter-month-and-line";
...
import ChartDefect from "@/app/(user)/andon/report/_components/chart-defect-time";
import ChartDefectByQty from "@/app/(user)/andon/report/_components/chart-defect-qty";
...
import { defectReportColumns } from "@/app/(user)/andon/report/_components/table-report-columns";
...
const fetchReportData = async (
  ...
  const [defectAccData, defectQtyAccData, andonReportData] = await Promise.all([
    ...
    getAndonReportData(targetMonth, targetOrg, targetLine),
  ]);
...
const AndonPage = async ({ searchParams }: { searchParams: SearchParams }) => {
  ...
  const { defectAccData, defectQtyAccData, andonReportData } =
  ...
  return (
    <div className="w-full space-y-4 rounded-lg bg-background p-4">
      <Header>
        <HeaderLeft>
          <HeaderSubtitle>Andon Report</HeaderSubtitle>
